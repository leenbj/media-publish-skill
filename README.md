...
- **标题规则**：企业新闻标题由 LLM 围绕“公司官网启用域名”主题自由拟（`gen_title_llm`），不设任何格式或风格限定；
  仅硬要求含企业全称或简称，另卡平台上限字数（头条 30 / 搜狐 72 / CSDN 100 最严上限）与已发标题去重，
  两次不合格才退回 `common.build_title` 兜底。观点文是独立命题，标题按自身内容生成，不强行套用企业新闻规则。
