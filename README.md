...
- 未配置任何 LLM 命令时只生成确定性兜底稿；正式发布前应配置两阶段命令，不能把兜底稿称为已人化。
...
1. 首段直接交代“企业全称官网启用‘域名’”，说明 `.网址` 对官网识别、用户访问和品牌名称对应关系的意义。
2. 第二段集中介绍联网检索到的公开企业资料，写清来源归属、成立时间、所在地、业务、产品或设施；不补写搜索摘要之外的经营数据、客户评价或现场细节。
3. 第三部分先联系企业所在行业的线上信息、服务或供应链场景，再使用 [references/url-material.md](references/url-material.md) 中的 `.网址` 背景资料，转到本企业登记使用该域名的识别、访问和资料统一价值；不承诺流量、排名、销量、平台效果或法律结果。
4. 末段回到启用事实，以适度的描述性语言总结，并标明产品、资质、服务和经营数据仍以官网或企业正式发布为准。
...
- **标题规则**：放弃旧的固定拼接方案。企业新闻标题只需出现企业全称或简称，并补充自然的新闻动作或事实角度，
  例如“企业简称官网入口更新”“企业名称主营产品调整”，不强制把完整域名塞进标题。完整域名和“官网启用”事实放在正文首段说明。
  角度从正文实际命中的事实标签中选择，未在正文出现的角度不得硬塞。按头条 30 / 搜狐 72 / CSDN 100 字最严上限裁剪。
  平台上限不足时才用表格简称或官方缩写，仍放不下则保留可识别的名称前缀并告警。
  观点文标题同样按文章内容（所选观点角度）命题，且必须含企业全称或简称，格式“企业名：观点角度”，字数放不下时换角度/更短名称。
- **新闻正文规格**：正文至少 1000 字，按“启用事实与入口意义 → 企业公开资料 → 行业数字化与 `.网址` 背景 → 总结边界”组织；
  第一段须含 `公司全称 + 官网启用 + 域名` 三要素。标题只要求企业全称或简称加新闻动作，不写成字段堆砌，不虚构引语。
...
- **第二篇观点文**：每行额外生成一篇独立科普文，与企业案例脱钩，只谈.网址本身；
  7 角度 × 3 变体，标题含企业名与中文.网址，空位池取模 21 格连发不重样；正文约千字，
