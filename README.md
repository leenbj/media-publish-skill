...
- **标题规则**：企业新闻标题由 LLM 拟写（`gen_title_llm`），参考“从‘云岭翻译.网址’看小语智能的数字品牌布局之道”这类
  描述性标题的风格自由发挥，不设任何固定样式；硬要求仅含企业全称或简称，另卡平台上限字数
  （头条 30 / 搜狐 72 / CSDN 100 最严上限）与已发标题去重，两次不合格才退回 `common.build_title` 兜底。
