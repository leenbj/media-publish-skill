...
- **标题规则**：企业新闻标题采用“域名：描述性短语”形式（如“中航成飞.网址：成飞航空科技的数字品牌战略支点”），
  由 LLM 拟写（`gen_title_llm`）：冒号前为完整域名，冒号后描述企业数字品牌/官网入口的意义，须含企业全称或简称；
  另卡平台上限字数（头条 30 / 搜狐 72 / CSDN 100 最严上限）与已发标题去重，两次不合格才退回 `common.build_title` 兜底。
  观点文是独立命题，标题按自身内容生成，不强行套用企业新闻规则。
