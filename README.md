...
- **标题规则**：`名称 + 官网启用 + 域名 + 短描述`，按本行目标媒体的最严上限裁剪
  ...
  （…有限公司→…集团）> 地域品牌截断（如 …新能源发展有限责任公司→…集团）；
  描述位用描述性语言，按正文主题命中选择（品牌保护/数字化升级/
  互联网转型/中文直达/安全升级/服务升级），多命中按行号轮换；
  无主题回退中性词；不用广告语，装不下直接省略。
- **正文首段**：须含 `公司全称 + 启用 + 域名` 三要素，位置不限（缺失才补）。
- **稿件审核**（③）：代码字符（HTML/Markdown/链接/JSON/占位符）、烂尾断句、
  .网址外域名后缀（含 com.cn/info/biz/cc/tv/io/ai 等）、.网址定向负面
  （保护语境如防钓鱼不误伤）→ 硬错整行跳过；通顺可疑只告警。
  表格加“简称”列即可强制指定简称（如 中国长江三峡集团）。
- **正文首段**：第一句必须是全称 `公司全称官网启用“域名”。`（标题用简称时正文仍亮全称），LLM 漏写自动补。
