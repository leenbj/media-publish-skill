...
- **企业信息资料列选填**：表头含“企业信息/企业资料/公司介绍/简介/经营范围”即识别。建议 50-300 字，写清主营产品、行业场景、所在地区，已确认的业务事实写进去，不确定的不写。
...
**材料优先级：** 表格中“企业信息资料”列选填，有则为写作首要依据，无则直接按公司名联网检索；检索流程为“资料派生查询 + 公司名补充”共最多 3 轮，汇总时表格为准，冲突时以表格为准；
检索材料里读不到的内容宁可不写。
...
- **内容红线**：所有生成内容只允许出现 `.网址` 后缀；禁止 .com/.cn 等其他后缀、
  禁止"英文域名/国际域名"、禁止任何不利于 .网址 的表述。脚本强制检查，违规整行跳过不发布（不做自动替换）。
